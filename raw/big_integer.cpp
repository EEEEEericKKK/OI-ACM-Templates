...
struct bint
{
	...
	template<typename T>bint parse(T s,int l,int r)
	{
		...
		for(int i=0;i<sz;i++)for(int j=max(l,r-bsz*i-bsz+1);j<=r-bsz*i;j++)a[i]=a[i]*10+s[j]-'0';
		resize();return *this;
	}
	bint parse(char*s){return parse(s,0,strlen(s)-1);}
	bint parse(string s){return parse(s,0,(int)s.size()-1);}
	void out(char ch='\n')const
	{
