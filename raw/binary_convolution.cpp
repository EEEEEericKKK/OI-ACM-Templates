...
namespace bin_conv
{
	...
	int ta[18][1<<17],tb[18][1<<17],tc[18][1<<17];
	void sub_conv(int *a,int *b,int *c,int n)
	{
		memset(ta,0,sizeof(ta));memset(tb,0,sizeof(tb));memset(tc,0,sizeof(tc));
		for(int i=0;i<(1<<n);i++)
		{
			ta[bits[i]][i]=a[i];
			tb[bits[i]][i]=b[i];
		...
		for(int i=0;i<=n;i++)
		{
			for(int j=0;j<=i;j++)
			{
				int*t1=ta[j],*t2=tb[i-j],*t3=tc[i];
				for(int k=0;k<(1<<n);k++)t3[k]=(t3[k]+1ll*t1[k]*t2[k])%mod;
			}
		}
		for(int i=0;i<=n;i++)fmt(tc[i],n,1);
		for(int i=0;i<(1<<n);i++)c[i]=tc[bits[i]][i];
	}
	...
	void prework(){for(int i=0;i<(1<<poly_max);i++)bits[i]=__builtin_popcount(i);}
};
int main()
{
	return 0;
