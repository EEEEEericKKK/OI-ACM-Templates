...
int Minimum_Arborescence(int rt,int n,int m)
{
	...
	memset(use,0,sizeof(use));dfs(rt);
	cnt=0;
	for(int i=1;i<=n;i++)if(e[ans_e[i]].v==n)cnt++;
	return cnt;
}
...
bool check(int val)
{
	for(int i=m-n+2;i<=m;i++)e[i].w=val;
	int tmp=Minimum_Arborescence(n,n,m);
	return (tmp>=k);
}
...
int main()
{
	...
	for(int i=1;i<=n;i++)e[++m]=edge(i,n+1,0);
	n++;
	
	int lb=0,rb=1e9+2,mid;
	while(lb<rb-1)
	{
		int mid=(lb+rb)>>1;
		if(check(mid))lb=mid;
